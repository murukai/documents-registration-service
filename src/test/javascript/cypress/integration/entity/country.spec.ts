...
describe('Country e2e test', () => {
  ...
  it('should create an instance of Country', () => {
    ...
    cy.get(`[data-cy="countryCode"]`).type('DE', { force: true }).invoke('val').should('match', new RegExp('DE'));

    cy.get(`[data-cy="callingCode"]`).type('Swedish', { force: true }).invoke('val').should('match', new RegExp('Swedish'));

    cy.get(`[data-cy="subRegion"]`).type('Frozen', { force: true }).invoke('val').should('match', new RegExp('Frozen'));

    cy.get(`[data-cy="region"]`).type('Savings plum', { force: true }).invoke('val').should('match', new RegExp('Savings plum'));

    cy.get(`[data-cy="population"]`).type('88433').should('have.value', '88433');

    cy.get(`[data-cy="timeZone"]`).type('open-source Chi', { force: true }).invoke('val').should('match', new RegExp('open-source Chi'));

    cy.get(`[data-cy="numericCode"]`).type('59691').should('have.value', '59691');
