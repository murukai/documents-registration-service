...
/**
 * REST controller for managing {@link com.afrikatek.documentsregistrationservice.domain.Applicant}.
 */
@RestController
@RequestMapping("/api")
public class ApplicantResource {
    ...
    @GetMapping("/applicants/{appointmentSlotId}/appointment-slot")
    public ResponseEntity<List<ApplicantDTO>> getApplicantsByAppointmentSlot(@PathVariable Long appointmentSlotId) {
        log.debug("REST request to get Applicants for a specific appointment slot : {}", appointmentSlotId);
        AppointmentSlotDTO appointmentSlotDTO = appointmentSlotService.findOne(appointmentSlotId).orElseThrow(EntityNotFoundException::new);
        Optional<List<ApplicantDTO>> applicantDTOs = applicantService.findAllByAppointmentSlot(appointmentSlotDTO);
        return ResponseEntity.ok().body(applicantDTOs.get());
    }
