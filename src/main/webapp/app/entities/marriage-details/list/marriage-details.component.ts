...
export class MarriageDetailsComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.marriageDetailsService
        ...
        .subscribe(
          (res: HttpResponse<IMarriageDetails[]>) => {
  ...
  trackId(index: number, item: IMarriageDetails): number {
    return item.id!;
  }

  delete(marriageDetails: IMarriageDetails): void {
    const modalRef = this.modalService.open(MarriageDetailsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.marriageDetails = marriageDetails;
  ...
  protected onSuccess(data: IMarriageDetails[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/marriage-details'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
