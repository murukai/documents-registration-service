...
export class AppointmentSlotComponent implements OnInit {
  ...
  trackId(index: number, item: IAppointmentSlot): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IAppointmentSlot[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/appointment-slot'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
